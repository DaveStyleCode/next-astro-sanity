...
Given the project requirements — Sanity-driven page builder, LaunchDarkly personalization with full-route A/B variants, a React Native mobile app sharing logic, and a team already invested in React — **Next.js is the stronger choice for the initial build**, with the understanding that an Astro migration remains a viable (and relatively mechanical) option down the road if the site's needs shift more toward pure content delivery.
...
**Verdict: Astro wins on raw static performance.** For a content-heavy house listing site, the zero-JS default is compelling. However, Next.js 15's Partial Prerendering narrows this gap significantly, and the personalization requirements (see below) mean many pages won't be purely static anyway.
...
Astro's **Server Islands** (stable since Astro 5) are worth addressing directly because they change the A/B testing story. A Server Island is a component that renders on the server on every request, even when the rest of the page is statically generated. The static shell is served instantly from the CDN, and the server island streams in its personalized content with a fallback/placeholder shown while it resolves.
...
This is a genuinely compelling pattern. It combines Astro's zero-JS static performance with per-request server-side personalization — something Next.js achieves with Partial Prerendering (PPR), which is conceptually very similar but still experimental in Next.js 15.

#### Updated Comparison

|                                   | Next.js                                                                                                                                                                                                                                | Astro (with Server Islands)                                                                                                                                                                                                                                                            |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Full-route variants**           | Middleware rewrites to `/page?variant=B` → same route renders different content based on server-side flag resolution. The page is a single React tree; swapping sections is trivial.                                                   | **Two good options:** (1) Middleware rewrites work the same way for fully SSR pages. (2) Server Islands let you statically generate the page shell and defer only the personalized sections to per-request server rendering. The static shell loads instantly; the variant streams in. |
| **Section-level variants**        | The `BlockRenderer` already handles this. Pass different `sections[]` arrays based on the resolved variant. One component, one render path.                                                                                            | A Server Island wrapping a React `BlockRenderer` (via `server:defer`) works cleanly. The LD flag is resolved server-side inside the island, the correct `sections[]` are fetched from Sanity, and the React component renders the blocks. This is architecturally sound.               |
| **Static + personalized hybrid**  | Next.js PPR (experimental) can serve a static shell and stream in dynamic holes. Conceptually identical to Server Islands but not yet stable. Without PPR, the entire page is either static or dynamic — no per-component granularity. | **Astro's strength.** Server Islands are the stable, production-ready version of this pattern. A page can be 90% static (cached at the CDN) with a single server island that resolves personalization. Best of both worlds.                                                            |
...
| **Caching personalized pages**    | With PPR (experimental): static shell is cached, dynamic holes are not. Without PPR: personalized pages must either use ISR with cookie-based keys (complex) or forgo caching entirely.                                                | Server Islands are excluded from the page cache by design. The static shell is fully cached at the CDN; the island is always server-rendered fresh. This is actually cleaner than Next.js's current stable caching story for personalized content.                                     |
...
Server Islands meaningfully close the gap. The pattern of "static shell + server-deferred personalized content" is elegant, and Astro has it stable today while Next.js's equivalent (PPR) is still experimental. For **server-side-only A/B testing** — where the variant is resolved per-request and the user never switches variants mid-session — Astro with Server Islands is arguably the better architecture. You get the CDN-cached static shell AND per-request personalization without shipping any JavaScript for the personalization logic.

However, the gap doesn't fully close for two reasons:

1. **Client-side reactivity.** If the user's segment changes mid-session (e.g., they log in, or a client-side event updates their LD context), Server Islands can't re-render without a page navigation. In Next.js, the React tree re-renders in place. Whether this matters depends on the UX requirements — for many A/B tests, server-side-only is sufficient.

2. **The BlockRenderer is React.** The Server Island would ultimately render a React component (`BlockRenderer`) server-side. This works, but it means the island is still paying the React server-rendering cost. The Astro advantage (no framework overhead) only applies to the non-personalized parts of the page.

**Verdict: Closer than initially stated — call it a slight Next.js advantage instead of a clear win.** If A/B testing is strictly server-side (flag resolved per-request, no client-side variant switching), Astro Server Islands are a strong pattern with better caching semantics than Next.js's current stable offering. Next.js retains the edge for client-side reactivity and the fact that the existing `BlockRenderer` already works without any wrapping. The real question is whether your A/B tests need client-side variant switching — if they don't, this criterion is close to a draw.
...
| LaunchDarkly Personalization | 15%    | 9/10    | 7/10   | Server-side is equal (Server Islands help); client-side React is superior                 |
| Build Times                  | 5%     | 7/10    | 9/10   | ISR offsets Astro's faster cold builds                                                    |
| Interactivity                | 10%    | 9/10    | 6/10   | House listing site needs significant interactivity                                        |
| Route-Level A/B Testing      | 15%    | 9/10    | 7.5/10 | Server Islands close the gap for server-side A/B; client-side switching still favors Next |
...
1. **Personalization is a first-class requirement, not an afterthought.** LaunchDarkly-driven full-route A/B testing with per-segment variants means most high-value pages have dynamic elements. Astro's Server Islands offer an elegant pattern here (static shell + server-deferred personalized content), but the moment you need client-side variant reactivity or cross-island state, you're back in React-land. Next.js's unified React tree handles both server-resolved and client-reactive personalization without architectural seams.
...
- The caching story mattered most: Astro's Server Islands have cleaner cache semantics for "mostly static + a few personalized holes" than Next.js's current stable offering (PPR is still experimental)
